...
import createShop from "@/backend/shops/createShop"
import addShopServices from "@/backend/shops/addServices.ts"
import addShopResources from "@/backend/shops/addResources.ts"
import completeOnboarding from "@/backend/shops/completeOnboarding.ts"
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (selectedResources.length > 0) {
          await addShopResources(shopId, selectedResources)
        }

        // 3️⃣ Insert services with prices
        const servicesPayload = selectedServices.map(service => ({
          service_name: service,
          price: servicePrices[service],
        }))

        await addShopServices(shopId, servicesPayload)
