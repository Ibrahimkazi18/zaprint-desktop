...
export class PrinterService {
  ...
  constructor() {
    ...
    console.log('[PrinterService] Platform:', os.platform());
  }
  ...
  async detectSystemPrinters(): Promise<SystemPrinter[]> {
    ...
    console.log(`[PrinterService] Detecting printers on ${platform}...`);
    
    try {
      let printers: SystemPrinter[] = [];
      
      if (platform === 'win32') {
        printers = this.detectWindowsPrinters();
      } else if (platform === 'darwin') {
        printers = this.detectMacPrinters();
      } else if (platform === 'linux') {
        printers = this.detectLinuxPrinters();
      } else {
        console.warn('[PrinterService] Unsupported platform:', platform);
        return [];
      }
      
      console.log(`[PrinterService] ✓ Detection complete: ${printers.length} printer(s) found`);
      return printers;
    } catch (error) {
  ...
  private detectWindowsPrinters(): SystemPrinter[] {
    try {
      console.log('[PrinterService] Running PowerShell Get-Printer command...');
      ...
      if (!output || output.trim() === '') {
        console.log('[PrinterService] No printers found in Windows');
        return [];
      }
      ...
      console.log(`[PrinterService] PowerShell returned ${printers.length} printer(s)`);
      ...
      try {
        ...
        if (defaultPrinter) {
          console.log('[PrinterService] Default printer:', defaultPrinter);
        }
      } catch (e) {
      ...
      // Log each printer
      result.forEach((p: SystemPrinter) => {
        console.log(`[PrinterService]   → ${p.name} (${p.status})${p.isDefault ? ' [DEFAULT]' : ''}`);
      });
  ...
  private mapWindowsStatus(status: number): 'online' | 'offline' | 'error' {
    ...
    console.log(`[PrinterService] Mapping Windows status: ${status}`);
  ...
  mapToAppPrinters(
    ...
    console.log('[PrinterService] Mapping system printers to registered printers...');
    console.log(`[PrinterService] System printers: ${systemPrinters.length}`);
    console.log(`[PrinterService] Registered printers: ${registeredPrinters.length}`);
    
    const systemPrinterMap = new Map(
      systemPrinters.map(p => [p.name.toLowerCase(), p])
    );

    const result = registeredPrinters.map(appPrinter => {
      ...
      if (systemPrinter) {
        ...
        console.log(`[PrinterService] ✓ Matched: "${appPrinter.printer_name}" → ${systemPrinter.status}`);
        return {
          ...appPrinter,
        ...
        console.log(`[PrinterService] ✗ Not found: "${appPrinter.printer_name}" → offline`);
        return {
          ...appPrinter,
  ...
  startMonitoring(
    ...
    console.log(`[PrinterService] Check interval: ${intervalMs}ms (${intervalMs / 1000}s)`);
    ...
    console.log('[PrinterService] ✓ Monitoring started');
  }
  ...
  private async checkAndUpdatePrinters(registeredPrinters: AppPrinter[]) {
    try {
      console.log('[PrinterService] === Monitoring Cycle Start ===');
      ...
      if (hasChanges || this.lastKnownPrinters.size === 0) {
        console.log('[PrinterService] Status changed detected, notifying...');
        ...
        console.log('[PrinterService] No changes detected');
      }
      
      console.log('[PrinterService] === Monitoring Cycle End ===');
    } catch (error) {
  ...
  private detectChanges(currentPrinters: SystemPrinter[]): boolean {
    if (currentPrinters.length !== this.lastKnownPrinters.size) {
      console.log(`[PrinterService] Printer count changed: ${this.lastKnownPrinters.size} → ${currentPrinters.length}`);
      return true;
    }
    ...
    for (const printer of currentPrinters) {
      ...
      if (!lastKnown || lastKnown.status !== printer.status) {
        console.log(`[PrinterService] Status changed for "${printer.name}": ${lastKnown?.status || 'new'} → ${printer.status}`);
        return true;
      }
    }
  ...
  stopMonitoring() {
    ...
    console.log('[PrinterService] ✓ Monitoring stopped');
  }
