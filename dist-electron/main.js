...
class PrinterService {
   ...
   */
  mapToAppPrinters(systemPrinters, registeredPrinters) {
   ...
   */
  startMonitoring(registeredPrinters, onStatusChange, intervalMs = 1e4) {
   ...
   */
  async checkAndUpdatePrinters(registeredPrinters) {
    try {
...
function printWebContents(win, deviceName, copies, printBackground) {
  return new Promise((resolve, reject) => {
...
async function waitForPrintReady(win, filePath) {
  ...
  while (win.webContents.isLoading()) {
    await new Promise(
      (resolve) => win.webContents.once("did-stop-loading", () => resolve())
    );
  }
...
async function printPdfAsImages(filePath, deviceName, copies) {
  ...
  const renderPromise = new Promise((resolve, reject) => {
    const cleanup = () => {
      electron.ipcMain.removeAllListeners(readyChannel);
      electron.ipcMain.removeAllListeners(errorChannel);
    };
    electron.ipcMain.once(readyChannel, () => {
      cleanup();
      resolve();
    });
    electron.ipcMain.once(errorChannel, (_event, message) => {
      cleanup();
...
function buildPdfPrintHtml(filePath, jobId) {
  return `<!doctype html>
<html>
  <head>
    <meta charset="utf-8" />
