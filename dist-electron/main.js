...
function setupPrinterHandlers(mainWindow2) {
  ...
  electron.ipcMain.handle("printer:test-print", async (_, { printerName, shopName, printerType }) => {
    try {
...
async function printFile(job, fallbackPrinters = []) {
  ...
  const fileExtension = path__namespace.extname(job.filePath).toLowerCase();
...
function selectPrinter(printers, requestedMode, systemPrinters) {
